...
void main_test_similarities(vector<Chord> data) {
    ...
    // Rahn
    set<double> rahnset = {0.75, 0.8, 0.85, 0.90, 0.95};
    ...
    // Lewin
    set<double> lewinset = {0.75, 0.8, 0.85, 0.90, 0.95};
    ...
    // Teitelbaum
    for(double thres=0; thres<=3; thres+=0.5) {
        FOR(i,data.size()) {
            FOR(j,data.size()) {
                matrixb[i][j] = similar(data[i], data[j], 7, thres);
            }
        }
        print_matrix_aux(matrixb, data, "draws/Teitelbaum("+to_string((int)(thres*10))+").txt");
        list << "Teitelbaum("+to_string((int)(thres*100))+").txt" << endl;
        pid = fork();
        if(pid != 0) {
            string s = "java -jar DrawMatrices.jar draws/Teitelbaum\\("+to_string((int)(thres*10))+"\\).txt";
            std::system(s.c_str());
            exit(0);
        }
    }
...
/*cout << "coucou entre 0 et " << last << endl;
FOR(i,rotation_table.size()) {
    cout << "[";
    FOR(j,rotation_table[i].size()) {
        cout << rotation_table[i][j] << "  ";
    }
    cout << "]" << endl;
}*/
...
//cout << "on va supprimer " << i << endl;
        ...
        if ((last==rotation_table[0].size()-1) && (rotation_table.size()>1)) {
            rotation_table.erase(rotation_table.begin()+1, rotation_table.end());
        }
//cout << endl;
    }
    set<unsigned> res;
...
double rahn(Chord c1, Chord c2) {
    ...
    double card1=PCS(c1).size(), card2=PCS(c2).size();
