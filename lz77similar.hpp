...
pair<unsigned, unsigned> longest_prefix_sim(vector<Chord> input, vector<Chord> dictionary);
...
vector<tuple<unsigned, unsigned, unsigned, Chord> > compress77_sim(vector<Chord> input, unsigned l_buf, unsigned l_prev);
...
vector<vector<Chord> > print_dictionary_sim(vector<tuple<unsigned, unsigned, unsigned, Chord> > dico, unsigned l_buf, unsigned l_prev);
...
map<string, unsigned> allSequences_sim(vector<Chord> input, unsigned threshold);
...
bool similar(Chord c1, Chord c2, unsigned metric, double threshold=0);
